...
// ============================================================================
class Indexer {
    static void walk(const fs::path& dir, FileList& list, int& skipped) {
        std::error_code ec;
        fs::directory_iterator it(dir, fs::directory_options::skip_permission_denied, ec);
        if (ec) { ++skipped; return; }
        for (auto end = fs::directory_iterator(); it != end; it.increment(ec)) {
            ...
            const bool dontDescend = attrs == INVALID_FILE_ATTRIBUTES || (attrs & FILE_ATTRIBUTE_REPARSE_POINT);
#else
            const bool dontDescend = entry.is_symlink(e2);
            if (e2) { ++skipped; continue; }
#endif
            list.append(entry.path().string());
            if (isDir && !dontDescend)
                walk(entry.path(), list, skipped);
        }
    }
public:
    static void buildIndex(const std::string& rootPath, FileList& list) {
        ...
        if (!fs::exists(root, ec) || ec) { std::cout << "Indexer: path does not exist: " << rootPath << "\n"; return; }
        if (fs::is_regular_file(root, ec) && !ec) { list.append(root.string()); std::cout << "Indexer: indexed 1 entry (single file): " << rootPath << "\n"; return; }
        int skipped = 0;
        walk(root, list, skipped);
        std::cout << "Indexer: indexed " << list.getCount() << " entr" << (list.getCount() == 1 ? "y" : "ies") << " under \"" << rootPath << "\"" << (skipped > 0 ? ("; skipped " + std::to_string(skipped) + " unreadable entr" + (skipped == 1 ? "y" : "ies") + ".") : ".") << "\n";
    }
};
